...
def linear_decay_function(initial_value: float,
    ...
    The parameter value changes linearly from the initial value to the final value over the decay range.
    ...
    If the final value is less than the initial value, the parameter value will decrease over the decay range.
    If the final value is greater than the initial value, the value will increase.
    
    `decay_start : int` - The inclusive start step of the decay range.
    
    `decay_stop : int` - The exclusive stop step of the decay range.
    
    `decay_rate : float = 1.0` - The decay rate, this is the factor of the decay range to which the parameter approaches its final value.
    For example, a decay rate of 0.5 will result in a line that approaches the final value half way between the decay start and decay end bound.
    ...
    """
    if decay_rate < 0.0:
        raise ValueError(f"Decay rate must be greater than 0.0. Got; {decay_rate}.")
    if decay_start > decay_end:
        raise ValueError(f"Decay start must be less than decay end. Got; {decay_start} > {decay_end}.")
...
def polynomial_decay_function(initial_value: float,
    ...
    The parameter value changes polynomially from the initial value to the final value over the decay range.
    Where the decay range is the range of steps between the decay start step (inclusive) and decay stop step (exclusive).
    ...
    Returns
    -------
    `(int) -> float` - A polynomial parameter decay function, returns the parameter value at a given iteration.
    The arguments given to this functions are assigned as attributes to the function instance returned.
...
def exponential_decay_function(initial_value: float,
    ...
    The parameter value changes exponentially from the initial value to the final value over the decay range.
    Where the decay range is the range of steps between the decay start step (inclusive) and decay stop step (exclusive).
    ...
    Returns
    -------
    `(int) -> float` - A exponential parameter decay function, returns the parameter value at a given iteration.
    The arguments given to this functions are assigned as attributes to the function instance returned.
...
def sinusoidal_decay_function(initial_value: float,
    ...
    The parameter value changes along a sinusoidal curve from the initial value to the final value over the decay range.
    Where the decay range is the range of steps between the decay start step (inclusive) and decay stop step (exclusive).
    ...
    Returns
    -------
    `(int) -> float` - A sinusoidal parameter decay function, returns the parameter value at a given iteration.
    The arguments given to this functions are assigned as attributes to the function instance returned.
...
def get_decay_function(decay_type: DecayFunctionType,
    ...
    If None, then a indentity function is created, which always returns the initial value.
    ...
    Returns
    -------
    `(int) -> float` - The decay function, returns the parameter value at a given iteration.
